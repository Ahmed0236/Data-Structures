...
// insertion helper
template <class T>
node<T>* BST<T> :: insertHelper(T k, node<T> *p) { // note we need a helper because we need recursive calls
	...
	{
		p->left = insertHelper(k, p->left);
	}
	else if (p->key < k)
	{
		p->right = insertHelper(k, p->right);
	}
...
// // 	// //cout << cs.height(ptr) << endl << endl;
// // 	// //cs.delete_node(1);
// 	cout << "height of root " << cs.height(ptr) << endl << endl;
// // 	// //cs.removemin(ptr);
